...
pub fn pretty_centiseconds(num: i64) -> String {
    ...
    [if is_negative {
     ...
     if DSECONDS_YEARS <= num {
         Cow::Owned(format!("{}y ", years))
     } else {
         Cow::Borrowed("")
     },
     if DSECONDS_DAYS <= num {
         Cow::Owned(format!("{}d ", days))
     } else {
         Cow::Borrowed("")
     },
     if DSECONDS_HOURS <= num {
         Cow::Owned(format!("{:02}:", hours))
     } else {
         Cow::Borrowed("")
     },
     if DSECONDS_MINUTES <= num {
         Cow::Owned(format!("{:02}:", minutes))
     } else {
         Cow::Borrowed("")
     },
